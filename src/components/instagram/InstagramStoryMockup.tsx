...
export function InstagramStoryMockup({
  ...
  const storyMediaUrl = post.videoUrl || post.mediaUrl;
  const isVideo = Boolean(storyMediaUrl && /\.(mp4|webm|mov|m4v)(\?|$)/i.test(storyMediaUrl));
  return (
    <div className="rounded-3xl border border-border bg-card p-3 shadow-soft">
      <div className="relative mx-auto aspect-[9/16] max-h-[620px] overflow-hidden rounded-[2rem] bg-muted">
        {isVideo ? (
          <video
            src={storyMediaUrl ?? ""}
            poster={post.videoPosterUrl || post.mediaUrl || undefined}
          ...
          <img src={post.videoPosterUrl || storyMediaUrl} alt={post.title} className="h-full w-full object-cover" />
        ) : (
