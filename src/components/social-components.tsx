...
export function PostPreview({ post }: { post: SocialPost }) {
  ...
  const lowerFormat = post.format.toLowerCase();
  const isVideo = lowerFormat.includes("reels") || lowerFormat.includes("video") || lowerFormat.includes("vídeo");
  if (isVideo) {
    return (
      <VideoReviewPlayer
        title={post.title}
        videoUrl={post.videoUrl}
        posterUrl={post.videoPosterUrl || post.mediaUrl}
        audioUrl={post.audioUrl || post.soundAssetUrl}
        soundProfile={post.soundProfile}
        compact
      />
    );
  }
...
export function PostCard({
  ...
  const mediaCount = isCarousel
    ...
    : isVideo
      ? (post.videoUrl ? 1 : 0) + (post.videoStoryboardUrls?.length || 0) || (post.mediaUrl ? 1 : 0)
      : post.mediaUrl
        ? 1
        : 0;
  return (
        ...
        <CardContent className="p-5">
          ...
          <div className="mt-3 grid gap-2 rounded-2xl border border-border bg-background p-3 text-xs text-muted-foreground sm:grid-cols-3">
            <span>
              ...
              {isCarousel ? "páginas" : isVideo ? "ativos vídeo" : "mídia"}
            </span>
            <span>
              <b className="text-foreground">{post.qualityScore || 0}</b>/100 score
            </span>
            <span className={isVideo ? (post.videoUrl ? "text-success" : "text-warning") : post.mediaUrl ? "text-success" : "text-warning"}>
              {isVideo ? (post.videoUrl ? "vídeo pronto" : "aguarda MP4") : post.mediaUrl ? "mídia pronta" : "aguarda mídia"}
            </span>
          </div>
      ...
      >
        <div className="bg-muted p-4">
          ...
          <div className="mt-4 rounded-2xl border border-border bg-card p-4 text-sm shadow-soft">
            <div className="flex flex-wrap gap-2">
              ...
              {post.videoUrl ? (
                <Badge className="bg-success/15 text-success hover:bg-success/15">MP4 pronto</Badge>
              ) : null}
            ...
            <p className="mt-2 text-muted-foreground">
              {post.format.toLowerCase().includes("carrossel")
                ...
                : post.format.toLowerCase().includes("reels") ||
                    post.format.toLowerCase().includes("vídeo") ||
                    post.format.toLowerCase().includes("video")
                  ? "Reels/Vídeo agora usa OpenAI Video API para gerar MP4 final com player, som e revisão antes da publicação."
        ...
        <div className="space-y-5 p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            {(post.format.toLowerCase().includes("reels") || post.format.toLowerCase().includes("video") || post.format.toLowerCase().includes("vídeo")) ? (
              <Button
                className="bg-gradient-primary text-primary-foreground"
                disabled={!onGenerateVideo}
...
export function FormatDistributionSelector({
  ...
  totalPosts?: number;
  onChange?: (formats: Record<ContentFormat, number>) => void;
}) {
  const currentTotal = Object.values(formats).reduce((acc, item) => acc + Math.max(0, Number(item || 0)), 0);
  const requestedTotal = Math.max(1, Number(totalPosts || currentTotal || 1));

  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-border bg-card p-3 text-sm">
        <span className="text-muted-foreground">Soma dos formatos</span>
        <div className="flex items-center gap-2">
          <Badge variant={currentTotal === requestedTotal ? "outline" : "destructive"}>
            {currentTotal}/{requestedTotal}
          </Badge>
          {currentTotal !== requestedTotal ? (
            <span className="text-xs text-destructive">Ajuste os campos ou altere o total. A IA receberá o total do botão.</span>
          ) : (
            <span className="text-xs text-success">Distribuição alinhada.</span>
          )}
        </div>
      </div>
                ...
                {onChange ? (
                  <Input
                    ...
                    onChange={(event) =>
                      onChange({
                        ...formats,
                        [format]: Math.max(0, Math.floor(Number(event.target.value) || 0)),
                      })
                    }
                    className="h-8 w-20 text-right"
                  />
                ) : (
                  <Badge variant="outline">{numericQuantity}</Badge>
                )}
              </div>
              <Progress className="mt-3" value={Math.min(100, (numericQuantity / requestedTotal) * 100)} />
            </div>
          );
        })}
      </div>
