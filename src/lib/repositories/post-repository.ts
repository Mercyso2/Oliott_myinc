...
function formatIsVideo(format?: string | null) {
  const value = String(format ?? "").toLowerCase();
  return value.includes("reels") || value.includes("video") || value.includes("vídeo");
}

function formatIsCarousel(format?: string | null) {
  return String(format ?? "").toLowerCase().includes("carrossel");
}

function inferJobType(post: PostRow, jobType?: "image" | "carousel" | "video") {
  if (jobType) return jobType;
  if (formatIsVideo(post.format)) return "video";
  if (formatIsCarousel(post.format)) return "carousel";
  return "image";
}
...
async function queueMediaBatch(token: string, payload: BatchPayload, mode: "image" | "video" | "mixed") {
  ...
  const response = await createProductionBatch(token, {
    brandId,
    postIds,
    jobType: mode === "video" ? "content" : undefined,
    force: payload.force ?? mode === "video",
    instruction:
      mode === "video"
        ? "Criar jobs de vídeo/Reels MP4 real no Motor Local usando OpenAI Video API, com áudio/direção sonora quando disponível."
        : mode === "image"
          ? "Criar jobs de imagem/carrossel no Motor Local com Creative Engine V7: base visual premium + render final com logo real, texto, paleta, CTA e camadas profissionais."
          : "Criar jobs de mídia no Motor Local com Creative Engine V7, render final de design, crítica de qualidade e uso do brand kit real.",
  });
  return {
    ok: true as const,
    processed: 0,
    ...
    message: response.queued
      ? `${response.queued} job(s) enviados para a fila do Motor Local V7. Acompanhe pela Fila V2.`
...
export async function generatePostImage(token: string, postId: string, jobType?: "image" | "carousel" | "video") {
  ...
  const resolvedJobType = inferJobType(post, jobType);
  const response = await createProductionBatch(token, {
    ...
    force: true,
    instruction:
      resolvedJobType === "video"
        ? "Gerar vídeo/Reels MP4 real com OpenAI Video API. Não substituir por imagem. Usar direção visual, marca, biblioteca, referência e som quando disponível."
        : resolvedJobType === "carousel"
          ? "Criar páginas de carrossel no Motor Local com narrativa editorial, base visual premium e render final com logo real, texto curto, CTA e consistência de coleção."
          : "Criar publicação final premium no Motor Local: base visual sem logo inventado + render final com logo real, headline, CTA, paleta e camadas MYINC.",
  });
  return {
    ok: true as const,
    queued: true,
    ...
    batchId: response.batchId,
    message: response.queued
      ? `${resolvedJobType === "video" ? "Vídeo/Reels" : "Mídia"} enviado para fila (${response.queued} job). Acompanhe na Fila V2 e atualize a prévia quando concluir.`
      : `${resolvedJobType === "video" ? "Vídeo/Reels" : "Mídia"} enviado para fila.`,
  };
}

export async function generatePostVideo(token: string, postId: string) {
  const post = await getActivePost(token, postId);
  if (!post.brand_id) throw new Error("Post sem brand_id para gerar vídeo.");

  await postRepository.update(token, postId, {
    status: "em_fila",
    media_url: null,
    video_url: null,
    video_poster_url: null,
    video_status: "em_fila",
    video_progress: 0,
    error_message: null,
    technical_detail: null,
    updated_at: new Date().toISOString(),
  } as unknown as Partial<PostRow>);

  return generatePostImage(token, postId, "video");
}
...
export function createProductionBatch(token: string, payload: { brandId: string; postIds: string[]; instruction?: string; jobType?: string; force?: boolean }) {
...
export async function hydratePostRelations(token: string, posts: PostRow[]) {
  ...
  let versions: PostVersionRow[] = [];
  let comments: ContentCommentRow[] = [];
  try {
    [versions, comments] = await Promise.all([
      postVersionRepository.list(token, `select=*&post_id=in.(${ids})&order=created_at.desc&limit=300`),
      contentCommentRepository.list(token, `select=*&post_id=in.(${ids})&order=created_at.desc&limit=300`),
    ]);
  } catch (error) {
    console.warn('Falha ao hidratar versões/comentários; seguindo sem travar a tela:', error);
  }
...
export function retryGenerationJobs(token: string, payload: { jobId?: string; postId?: string } = {}) {
  return callEdgeFunction<{ ok: true; requeued: number; jobs: GenerationJobRow[] }>("retry-generation-job", token, payload);
}

export function generateCarouselsBatch(token: string, payload: BatchPayload) {
  return queueMediaBatch(token, payload, "image");
}
