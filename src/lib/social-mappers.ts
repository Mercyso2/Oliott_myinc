...
function looksLikeVideoUrl(value: unknown) {
  ...
  return lower.includes(".mp4") || lower.includes(".mov") || lower.includes(".webm") || lower.includes("video/") || lower.includes("/videos/") || lower.includes("video_url=");
}
...
export function postRowToSocialPost(
  ...
  const metadata = rowAny.metadata && typeof rowAny.metadata === "object" ? (rowAny.metadata as Record<string, unknown>) : {};
  const rawVideoCandidate = firstString(rowAny.video_url, output.video_url, output.videoUrl, output.mediaUrl, output.media_url);
  const videoUrl = looksLikeVideoUrl(rawVideoCandidate) ? rawVideoCandidate : "";
  const videoPosterUrl = firstString(
    rowAny.video_poster_url,
    output.posterUrl,
    output.poster_url,
    looksLikeVideoUrl(row.media_url) ? "" : row.media_url,
    looksLikeVideoUrl(rawVideoCandidate) ? "" : rawVideoCandidate,
  );
  const audioUrl = String(rowAny.audio_url || output.audio_url || metadata.audio_url || "");
  const soundAssetUrl = String(rowAny.sound_asset_url || output.sound_asset_url || metadata.sound_asset_url || "");
  const soundProfile = String(output.sound_direction || metadata.sound_direction || "");
  const qualityReview =
  ...
  return {
    ...
    caption: row.caption ?? "",
    hashtags: arrayFromJson(row.hashtags),
    cta: row.cta ?? "",
