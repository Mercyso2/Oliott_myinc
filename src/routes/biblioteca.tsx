...
const uploadTypes = ["Logo", "Foto pessoal / retrato", "Referência visual", "Template", "Empreendimento", "Vídeo", "Som / Trilha", "Documento"];
...
function normalizeUploadType(file: File, selected: string) {
  ...
  if (file.type.startsWith("audio/")) return "Som / Trilha";
...
function usageRule(type: string) {
  ...
  if (type === "Som / Trilha") return "Usar como referência sonora para Reels: trilha, ritmo, clima, efeito de transição e identidade sonora. Não copiar música protegida; usar como direção criativa.";
...
function defaultStatus(type: string) {
  return type === "Logo" || type === "Foto pessoal / retrato" || type === "Som / Trilha" ? "referência aprovada" : "ativo";
}

function defaultAiAllowed(type: string) {
  return type === "Logo" || type === "Foto pessoal / retrato" || type === "Som / Trilha";
}
...
function Biblioteca() {
  ...
  async function upload(file: File) {
    ...
    await run("Arquivo enviado, classificado e disponível na biblioteca.", async () => {
      ...
      if (!file.type.startsWith("image/") && !file.type.startsWith("video/") && !file.type.startsWith("audio/") && file.type !== "application/pdf") throw new Error("Formato não suportado. Envie PNG/JPG/WebP/SVG, vídeo, áudio ou PDF.");
      ...
      const tagList = Array.from(new Set([
        ...
        finalType === "Som / Trilha" ? "som" : "",
        finalType === "Som / Trilha" ? "trilha" : "",
      ...
      const asset = await mediaRepository.create(session.access_token, {
        ...
        usage_context: finalType === "Logo" ? "logo_oficial" : finalType === "Foto pessoal / retrato" ? "foto_pessoal_autorizada" : finalType === "Som / Trilha" ? "sound_reference" : "biblioteca",
      ...
      await libraryRepository.create(session.access_token, {
        ...
        tags: tagList,
        notes: finalNotes,
        format: finalType === "Logo" ? "Logo oficial" : finalType === "Foto pessoal / retrato" ? "Foto pessoal" : finalType === "Som / Trilha" ? "Som/Reels" : "Todos",
        ai_usage_rule: rule,
        ai_allowed: aiAllowed,
        asset_role: finalType,
        usage_context: finalType === "Logo" ? "logo_oficial" : finalType === "Foto pessoal / retrato" ? "foto_pessoal_autorizada" : finalType === "Som / Trilha" ? "sound_reference" : "referência visual",
        metadata: { upload_type: finalType, file_name: file.name, mime_type: file.type, theme_policy: "light_only" },
  ...
  const sounds = items.filter((i) => String(i.type).toLowerCase().includes("som") || i.tags.includes("som") || i.tags.includes("trilha"));
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-7">
      <PageHeader
        ...
        actions={
          <>
            <input ref={inputRef} type="file" className="hidden" accept="image/png,image/jpeg,image/webp,image/svg+xml,video/*,audio/*,.pdf" onChange={(event) => event.target.files?.[0] && void upload(event.target.files[0])} />
      ...
      <Tabs defaultValue="todos" className="space-y-5">
        ...
        <TabsContent value="sons"><MediaLibraryGrid items={sounds} onApproveReference={(item) => run("Som liberado como referência de Reels.", () => rowByItem(item) ? markReferenceApproved(session!.access_token, item.id) : mediaRepository.update(session!.access_token, item.id, { status: "referência aprovada", ai_allowed: true } as Partial<MediaAssetRow>))} onArchive={(item) => run("Som arquivado.", () => rowByItem(item) ? libraryRepository.archive(session!.access_token, item.id, { status: "arquivado", ai_allowed: false } as Partial<LibraryItemRow>) : mediaRepository.archive(session!.access_token, item.id, { status: "arquivado", ai_allowed: false } as Partial<MediaAssetRow>))} onDelete={deleteItem} /></TabsContent>
