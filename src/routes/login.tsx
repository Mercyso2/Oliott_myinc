      ...
      <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-card/95 p-8 shadow-elevated backdrop-blur-xl">
        ...
        <div className="mt-8 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Entre com uma conta autorizada no Supabase Auth para acessar o painel MYINC.
          </p>
        </div>
        <form onSubmit={submit} className="mt-8 space-y-4">
          ...
          <label className="space-y-2">
            <span className="text-sm font-semibold">Credencial</span>
            <Input
              type="password"
              value={password}
          ...
          {!isSupabaseConfigured && (
            <div className="rounded-2xl border border-warning/30 bg-warning/10 p-3 text-sm text-warning">
              Supabase ainda não está configurado. Defina as variáveis públicas do projeto para autenticar usuários reais.
            </div>
          )}
          <Button
            type="submit"
