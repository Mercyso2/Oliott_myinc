...
function videoFrameCount(format = "") {
  const explicit = Number(env("VIDEO_STORYBOARD_FRAMES", "5"));
  if (Number.isFinite(explicit) && explicit > 0) return Math.min(8, Math.max(3, explicit));
  return String(format).includes("curto") ? 4 : 5;
}
...
function fallbackVideoFrames(post, count) {
  const scenes = [
    "hook visual de impacto nos 3 primeiros segundos com arquitetura contemporânea e luz cinematográfica",
    "cena de fachada, implantação ou entorno valorizado reforçando localização estratégica",
    "cena de detalhes de acabamento, textura, paisagismo ou material nobre",
    "cena de lifestyle premium, calma, urbana e aspiracional, sem pessoas em primeiro plano deformáveis",
    "cena final com composição limpa e espaço seguro para CTA elegante",
    "cena de bastidores de obra, precisão técnica e confiança da incorporadora",
    "cena de vista, cidade e valorização patrimonial",
    "cena de fechamento institucional sofisticado da MYINC",
  ];
  return Array.from({ length: count }, (_, index) => ({
    frame: index + 1,
    page: index + 1,
    title: index === 0 ? `Hook - ${String(post.title ?? "MYINC")}` : `Frame ${index + 1}`,
    text: index === count - 1 ? String(post.cta ?? "Fale com a MYINC") : String(post.headline ?? post.theme ?? "Narrativa visual premium"),
    visual_prompt: `${scenes[index]}; manter continuidade visual, mesma paleta e linguagem do reels; criar frame vertical premium sem texto`,
  }));
}
...
function buildVideoFramePrompt(ctx, frame, count, feedback = null) {
  const frameObject = object(frame);
  return [
    buildBaseImagePrompt(ctx, feedback),
    `REELS/VÍDEO MYINC - frame/storyboard ${frameObject.frame ?? frameObject.page ?? 1}/${count}.`,
    `Roteiro/papel do frame, sem escrever texto na imagem: ${compact(frameObject.text ?? ctx.post.video_prompt ?? ctx.latestOutput.video_script ?? ctx.post.caption ?? "", 900)}.`,
    `Direção visual específica: ${compact(frameObject.visual_prompt ?? ctx.post.video_prompt ?? ctx.post.image_prompt ?? ctx.post.creative_brief ?? "", 1000)}.`,
    "Gerar um frame vertical cinematográfico 9:16, com continuidade visual da campanha. Não gerar letras, números, legendas, marca d'água ou logotipo.",
  ].join("\n");
}

function isGptImageModelName(model = '') {
  return String(model).toLowerCase().startsWith('gpt-image');
}

function isDalleImageModelName(model = '') {
  return String(model).toLowerCase().startsWith('dall-e');
}

function normalizeGeneratedImageFormat(format = 'png') {
...
function normalizeOpenAiImageSize(model, requestedSize) {
  const { width, height } = parseRequestedImageSize(requestedSize);
  const square = Math.abs(width - height) < Math.max(width, height) * 0.08;
  const landscape = width > height;
  const m = String(model || '').toLowerCase();

  if (isGptImageModelName(m)) {
...
function normalizeOpenAiImageQuality(model, quality) {
...
function buildOpenAiImageBody(prompt, format) {
  const model = imageModel();
  const size = normalizeOpenAiImageSize(model, openAiSize(format));
  const quality = normalizeOpenAiImageQuality(model, env('OPENAI_IMAGE_QUALITY', 'medium'));
  const outputFormat = normalizeGeneratedImageFormat(env('OPENAI_IMAGE_FORMAT', 'png'));
  const base = { model, prompt, size, n: 1 };

  if (isGptImageModelName(model)) {
    return { body: { ...base, output_format: outputFormat, quality }, model, size, quality, outputFormat };
  }

  if (isDalleImageModelName(model)) {
    const body = { ...base, response_format: 'b64_json' };
    if (quality) body.quality = quality;
    return { body, model, size, quality, outputFormat: 'png' };
  }
...
function detectImageInfo(bytes, fallbackFormat = 'png') {
  ...
  if (bytes.byteLength < 64) throw new Error(`Imagem inválida: arquivo pequeno demais (${bytes.byteLength} bytes).`);

  const isPng = bytes.length > 24 && bytes[0] === 0x89 && bytes[1] === 0x50 && bytes[2] === 0x4e && bytes[3] === 0x47;
  if (isPng) {
    const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    return { mime: 'image/png', ext: 'png', width: view.getUint32(16), height: view.getUint32(20), format: 'png' };
  }

  const isJpeg = bytes.length > 3 && bytes[0] === 0xff && bytes[1] === 0xd8 && bytes[2] === 0xff;
  if (isJpeg) return { mime: 'image/jpeg', ext: 'jpg', width: 1024, height: 1024, format: 'jpeg' };
  ...
  if (isWebp) return { mime: 'image/webp', ext: 'webp', width: 1024, height: 1024, format: 'webp' };

  const preview = Buffer.from(bytes.slice(0, 80)).toString('utf8').replace(/\s+/g, ' ').slice(0, 80);
  throw new Error(`OpenAI retornou arquivo que não é imagem PNG/JPEG/WEBP. Início: ${preview}`);
}

async function generateImageBytes(prompt, format) {
  const request = buildOpenAiImageBody(prompt, format);
  ...
  if (!response.ok) {
    throw new Error(`OpenAI image HTTP ${response.status}: ${data?.error?.message || JSON.stringify(data)} | payload=${JSON.stringify({ model: request.model, size: request.size, quality: request.quality, output_format: request.outputFormat })}`);
  }

  let bytes = null;
  const encoded = object(Array.isArray(object(data).data) ? object(data).data[0] : {}).b64_json;
  const url = object(Array.isArray(object(data).data) ? object(data).data[0] : {}).url;
  if (encoded) {
    bytes = Uint8Array.from(Buffer.from(encoded, 'base64'));
  } else if (url) {
    const media = await fetch(url);
    if (!media.ok) throw new Error(`OpenAI retornou URL, mas download falhou: HTTP ${media.status}`);
    bytes = new Uint8Array(await media.arrayBuffer());
  }

  if (!bytes) throw new Error(`OpenAI não retornou b64_json nem URL. Resposta: ${JSON.stringify(data)}`);
  const info = detectImageInfo(bytes, request.outputFormat);
...
async function createMediaAsset({ ctx, job, label, bytes, info, path, publicUrl, prompt, assetType, pageNumber = null, isFinal = true }) {
  ...
  const isVideoFrame = assetType === "video_frame" || assetType === "video_thumbnail";
  const { data: mediaAsset, error } = await supabase
    ...
    .insert({
      ...
      media_type: assetType === "video" ? "video" : isVideoFrame ? "Storyboard de vídeo" : "Imagem gerada",
      bucket: MEDIA_BUCKET,
      path,
      url: publicUrl,
      ...
      status: "ativo",
      tags: isCarousel ? ["ia", "myinc", "carrossel", `pagina-${pageNumber}`] : isVideoFrame ? ["ia", "myinc", "reels", "storyboard", `frame-${pageNumber}`] : ["ia", "myinc", assetType],
...
async function processImageJob(job, pagePayload = null, assetTypeOverride = null) {
  const ctx = await loadPostContext(job.post_id, job.brand_id);
  const payload = object(job.payload);
  const page = pagePayload ?? payload.page ?? payload.frame ?? null;
  const expected = Number(payload.expected_pages ?? payload.expected_frames ?? carouselPageCount(ctx.post.format));
  const assetType = assetTypeOverride ?? (job.job_type === "video_frame" ? "video_frame" : page ? "carousel_page" : "image");
  const isVideoFrame = assetType === "video_frame";
  const prompt = isVideoFrame
    ? buildVideoFramePrompt(ctx, page, expected, payload.feedback)
    : page
      ? buildPagePrompt(ctx, page, expected, payload.feedback)
      : buildBaseImagePrompt(ctx, payload.feedback);

  await insertEvent(job.id, "provider_requested", isVideoFrame ? "Solicitando frame de vídeo à OpenAI." : "Solicitando imagem à OpenAI.", { model: imageModel(), size: openAiSize(ctx.post.format), assetType });
  const { bytes, info } = await generateImageBytes(prompt, ctx.post.format);
  const folder = isVideoFrame ? "video-storyboard" : page ? "carousel" : "feed";
  const { path, publicUrl } = await uploadBytes({ brandId: ctx.post.brand_id, postId: ctx.post.id, folder, bytes, info });
  const pageNumber = page ? Number(object(page).page ?? object(page).frame ?? 1) : null;
  await createMediaAsset({
    ctx,
    job,
    label: isVideoFrame ? `Reels ${ctx.post.title} - frame ${pageNumber}` : page ? `Carrossel ${ctx.post.title} - página ${pageNumber}` : `Criativo ${ctx.post.title}`,
    bytes,
    info,
    path,
    publicUrl,
  ...
  if (!page && !isVideoFrame) await createImageVersionAndUpdatePost({ ctx, job, mediaUrl: publicUrl, prompt });
  await updateJob(job.id, { status: "completed", progress: 100, result: { media_url: publicUrl, page: pageNumber, asset_type: assetType }, output_json: { media_url: publicUrl, page: pageNumber, asset_type: assetType }, finished_at: nowIso() });
  await insertEvent(job.id, "job_completed", isVideoFrame ? "Frame de vídeo concluído." : "Imagem concluída.", { media_url: publicUrl, page: pageNumber, assetType });
}
...
async function processVideoParent(job) {
  const directProvider = env("VIDEO_GENERATION_MODE", "storyboard") === "provider" && env("VIDEO_PROVIDER_API_URL") && env("VIDEO_PROVIDER_API_KEY");
  if (directProvider) {
    await processVideoDirect(job);
    return;
  }
  ...
  const ctx = await loadPostContext(job.post_id, job.brand_id);
  const count = videoFrameCount(ctx.post.format);
  const frames = fallbackVideoFrames(ctx.post, count);
  const rows = frames.map((frame, index) => ({
    brand_id: job.brand_id,
    post_id: job.post_id,
    parent_job_id: job.id,
    job_type: "video_frame",
    type: "video_frame",
    ...
    max_attempts: Number(job.max_attempts ?? 3),
    payload: {
      frame: { ...object(frame), frame: Number(object(frame).frame ?? index + 1), page: Number(object(frame).page ?? index + 1) },
  ...
  const { error } = await supabase.from("generation_jobs").insert(rows);
  if (error) throw error;
  await supabase.from("posts").update({ video_status: "storyboard_processing", video_progress: 10, status: "em_producao", updated_at: nowIso() }).eq("id", job.post_id);
  await updateJob(job.id, { status: "waiting_children", progress: 15, result: { expected_frames: count }, output_json: { video_frames: frames } });
  await insertEvent(job.id, "video_frames_created", `${rows.length} frames/storyboard do vídeo enfileirados.`, { count });
}

async function consolidateWaitingVideos() {
  const { data: parents, error } = await supabase.from("generation_jobs").select("*").eq("job_type", "video").eq("status", "waiting_children").limit(10);
  if (error) throw error;
  for (const parent of parents ?? []) {
    const { data: children, error: childrenError } = await supabase.from("generation_jobs").select("*").eq("parent_job_id", parent.id).order("created_at");
    if (childrenError) throw childrenError;
    ...
    const completed = children.filter((child) => child.status === "completed");
    const failed = children.filter((child) => child.status === "failed");
    const expected = Number(object(parent.result).expected_frames ?? children.length);
    const progress = Math.min(95, Math.round((completed.length / expected) * 100));
    await updateJob(parent.id, { progress });
    await supabase.from("posts").update({ video_status: "storyboard_processing", video_progress: progress, updated_at: nowIso() }).eq("id", parent.post_id);

    if (completed.length < expected) {
      if (failed.length && completed.length + failed.length >= expected) {
        await updateJob(parent.id, { status: "failed", error_message: "Um ou mais frames do vídeo falharam.", finished_at: nowIso() });
        await supabase.from("posts").update({ status: "erro_video", error_message: "Um ou mais frames do vídeo falharam.", updated_at: nowIso() }).eq("id", parent.post_id);
      }
      continue;
    }

    const { data: frameAssets, error: assetsError } = await supabase
      .from("generation_job_assets")
      .select("*")
      .in("job_id", completed.map((child) => child.id))
      .order("page_number");
    ...
    const urls = (frameAssets ?? []).sort((a, b) => Number(a.page_number ?? 0) - Number(b.page_number ?? 0)).map((asset) => asset.public_url).filter(Boolean);
    if (!urls.length) continue;
    ...
    const { data: version, error: versionError } = await supabase.from("post_versions").insert({
      ...
      version_label: `V${Date.now()}`,
      caption: ctx.post.caption,
      image_prompt: `Vídeo/Reels assíncrono com ${urls.length} frames/storyboard gerados pelo worker.`,
      media_url: urls[0],
      quality_score: ctx.post.quality_score,
      is_current: true,
      output_json: {
        media_url: urls[0],
        video_storyboard_urls: urls,
        video_frames: object(parent.output_json).video_frames ?? [],
        worker_id: WORKER_ID,
        parent_job_id: parent.id,
    ...
    await supabase.from("posts").update({
      media_url: urls[0],
      video_poster_url: urls[0],
      video_status: "storyboard_completed",
      video_progress: 100,
      current_version_id: version.id,
      status: "aguardando_revisao",
      error_message: null,
    ...
    await updateJob(parent.id, { status: "completed", progress: 100, result: { video_storyboard_urls: urls }, output_json: { video_storyboard_urls: urls }, finished_at: nowIso() });
    await insertEvent(parent.id, "job_completed", "Vídeo/Reels consolidado como storyboard.", { urls });
  }
}
...
async function processVideoDirect(job) {
  const ctx = await loadPostContext(job.post_id, job.brand_id);
  const prompt = [
...
async function failOrRetry(job, error) {
  ...
  if (!shouldRetry && job.post_id) {
    await supabase.from("posts").update({
      status: job.job_type === "video" || job.job_type === "video_frame" ? "erro_video" : job.job_type === "carousel" || job.job_type === "carousel_page" ? "erro_carrossel" : "erro_imagem",
      error_message: message,
      technical_detail: message,
      updated_at: nowIso(),
    }).eq("id", job.post_id);
...
async function handleJob(job) {
  ...
  try {
    ...
    else if (job.job_type === "video") await processVideoParent(job);
    else if (job.job_type === "video_frame") await processImageJob(job, object(job.payload).frame, "video_frame");
...
async function claimJob() {
  let response = await supabase.rpc("claim_generation_job", { worker_id: WORKER_ID });
  if (response.error && String(response.error.message ?? "").includes("function")) {
    response = await supabase.rpc("claim_next_generation_job", { p_worker_id: WORKER_ID });
  }
...
async function mainLoop() {
  ...
  while (true) {
    try {
      ...
      await consolidateWaitingVideos();
